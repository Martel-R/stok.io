...
export function ImportProductsDialog({
    ...
    const parseCSV = (csvText: string) => {
        ...
        const expectedHeaders = [
            'name', 'category', 'purchasePrice', 'price', 'lowStockThreshold', 
            'isSalable', 'barcode', 'imageUrl', 'code', 'stock'
        ];
        
        // A simple check for required headers, not all need to be present but the core ones should be
        if (!['name', 'category', 'price', 'purchasePrice'].every(h => headers.includes(h))) {
            toast({ title: 'Cabeçalhos inválidos', description: `O CSV deve conter pelo menos: name, category, price, purchasePrice.`, variant: 'destructive' });
            return;
        }
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            const isSalable = entry.isSalable?.toLowerCase() !== 'false';
    ...
    const downloadTemplate = () => {
        const headers = "name,category,purchasePrice,price,lowStockThreshold,isSalable,barcode,imageUrl,code,stock";
