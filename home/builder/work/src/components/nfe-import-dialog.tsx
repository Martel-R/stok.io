    ...
    const parseNfeXml = (xmlText: string) => {
        ...
        try {
            ...
            const nfeProducts: NfeProduct[] = detItems.map(item => {
                ...
                const rastro = prod.getElementsByTagName('rastro')[0]; // Check for the traceability group
                const expirationDateStr = rastro ? getTagValue(rastro, 'dVal') : ''; // Get dVal only if rastro exists
